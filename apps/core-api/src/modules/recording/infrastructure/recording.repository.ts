...
export class RecordingRepository extends DddRepository<Recording> {
  ...
  async find(
    conditions: { ids?: number[]; castingIds?: number[]; lineIds?: number[]; creatorId?: number },
  ...
  async count(conditions: { ids?: number[]; castingIds?: number[]; lineIds?: number[]; creatorId?: number }) {
