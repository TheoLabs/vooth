...
### 6. 검수(Review) — 검수자 전용 ✅ (구현)
- 검수 전용 화면(`/review`): 검수자가 대사별 녹음(take)을 듣고 **승인(=최종 take 선택 + DONE)** / **반려(REJECTED)**.
- 회차의 (녹음 있는) 모든 대사가 확정되면 **회차 최종 승인**.
- **최종 take 선택은 검수자가 한다**(승인 = 최종 선택 + 완료).
- 접근제어: "검수" 메뉴는 검수 권한 계정에만 노출(추후 `/me` 가 권한 포함하도록 백엔드 보강 필요. 현재는 mock).

### 7. 제출·알림·상태 동기화 등 (추후)
- 녹음 제출 → 검수 → 재녹음 요청 흐름의 상태 동기화(공용 스토어/백엔드), 알림 등.
